...
Flux exposes a small key-authenticated REST API at `/api/v1` (projects, tasks, time tracking) for external agents and integrations. Keys are minted by an admin at `/admin/api-keys` or with `scripts/mint-api-key.mjs`.

The reference lives in [API.md](API.md) and is served publicly, without a session, at **<https://flux.foodverse.io/api-docs>** (raw Markdown, CORS `*`) so other apps can fetch it. That route reads a build-time embed of `API.md` generated by `scripts/gen-api-docs.mjs` (wired to `predev`/`prebuild`) — edit `API.md` only; the generated module is git-ignored.
